...
function optionList(nodes: SectorData["sectors"]["nodes"]) {
  return nodes
    .filter((node): node is NonNullable<typeof node> => Boolean(node?.name && node.slug))
    .map((node) => ({
...
export default async function CompaniesPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const [directoryResult, sectorResult] = await Promise.all([
    queryWithFallback<DirectoryData>(
      {
      ...
      EMPTY_DIRECTORY,
      "server company directory",
    ),
    queryWithFallback<SectorData>(
      { query: GET_ALL_SECTORS },
      EMPTY_SECTORS,
      "company directory sectors",
    ),
  ]);

  const directory = directoryResult.data?.sektorelCompanyDirectory ?? EMPTY_DIRECTORY.sektorelCompanyDirectory;
  const companies = (directory.nodes ?? []).filter(
    (company): company is Company => Boolean(company?.id && company.slug),
  );
  const sectors = optionList(sectorResult.data?.sectors?.nodes ?? []);
  const selectedLocationName = location
    ? companies
        .flatMap((company) => company.locations?.nodes ?? [])
        .find((item) => item?.slug === location)?.name ?? undefined
    : undefined;
  const hasError = directoryResult.hasError || sectorResult.hasError;
        ...
        <div className="container relative z-10 mx-auto">
          <div className="max-w-3xl">
            <div className="inline-flex items-center gap-2 border border-white/10 bg-white/5 px-3 py-1 text-xs font-bold uppercase tracking-[0.3em] text-gray-300">
              <Building2 size={12} className="text-primary" /> Firma Rehberi
            </div>
            ...
            <p className="mt-4 max-w-2xl text-lg leading-8 text-gray-300">
              Sektör, şehir ve doğrulama filtreleriyle Türkiye genelindeki firmaları gerçek zamanlı olarak keşfedin.
            </p>
          </div>
        </div>
      </section>
      ...
      <div className="container mx-auto space-y-8 px-4 py-10">
        {hasError ? (
          <div className="border border-orange-200 bg-orange-50 px-4 py-3 text-sm font-medium text-orange-700">
            Bazı filtre seçenekleri alınamadı. Firma sonuçları mevcut verilerle gösteriliyor.
          </div>
        ) : null}
        ...
        <div className="grid gap-8 lg:grid-cols-[300px_minmax(0,1fr)]">
          <CompanyFilters
            ...
            selectedSort={sort}
            selectedVerified={verified ? "true" : ""}
          />
          ...
          <div className="space-y-8">
            ...
            <section className="space-y-4">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <p className="text-sm text-gray-500">{directory.total} sonuç bulundu</p>
              </div>
